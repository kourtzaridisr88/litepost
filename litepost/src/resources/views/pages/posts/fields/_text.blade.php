<div class="form-group">
    ...
    <input type="text" class="form-control form-control--small {{ $errors->has('slug') ? ' is-invalid' : '' }}" 
        id="{{ $customField->fieldable->name }}" 
        name="custom_fields[{{ $customField->fieldable->name }}]" 
        value="{{ isset($post) ? old('custom_fields[' . $customField->fieldable->name . ']', $post->field($customField->fieldable->name)) : old('custom_fields[' . $customField->fieldable->name . ']') }}"
    >
